...
**适用场景**：无扩展名的二进制可执行文件，如 Go 编译的程序、C 编译的工具等
- **支持格式**：无扩展名的可执行文件（需有执行权限）
- **直接执行**：绕过 Shell，直接启动二进制文件
- **后台运行**：不阻塞 SpearX 主界面
- **工作目录**：自动设置工具所在目录为工作目录
...
| ------ | ---------------- | ---------------------------- |
| **1**  | 用户指定的 VALUE | 按配置执行                   |
| **2**  | 文件扩展名检测   | `.jar` → Java, `.app` → Open, 无扩展名+可执行 → Binary |
| **3**  | 文件内容分析     | 检测文件头、执行权限         |
| **4**  | 系统默认关联     | 使用操作系统文件关联         |
...
- ToolName: 二进制工具
  PATH: /tools/bin
  FileName: mytool              # 无扩展名+可执行权限，自动检测为 Binary
  VALUE: ""
```
...
首次启动时自动创建默认配置，如有旧版本配置在 .app 包内将自动迁移。
...
**Q: 如何备份工具配置？**
> A: 配置文件存储在用户目录下，应用更新不会覆盖。备份以下内容：
> - macOS: `~/Library/Application Support/SpearX/tool.yml` - 工具配置
